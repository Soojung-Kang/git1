...
document.addEventListener("DOMContentLoaded", function () {
	...
	const teleprompterContent = teleprompter.querySelector(".content");
	let isScrolling = false;
	let scrollInterval;
	const scrollSpeed = 1; // 스크롤 속도 조절 가능

	// 초기 위치 설정: 80% 지점에서 시작
	teleprompter.scrollTop = teleprompterContent.scrollHeight * 1.2;

	// 스페이스 바 이벤트 리스너 추가
	document.addEventListener("keydown", function (event) {
		if (event.code === "Space") {
			event.preventDefault(); // 기본 스페이스 바 동작 방지
			if (isScrolling) {
				// 스크롤 멈춤
